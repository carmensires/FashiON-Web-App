...
public class Ejb {
	...
	{
		List<Amigo> listaAmigos=(List<Amigo>)em.createNamedQuery("Amigo.findSeguidores").setParameter("usuarioSeguido", idUsuario).getResultList();
	...
	{
		List<Amigo> listaAmigos=(List<Amigo>)em.createNamedQuery("Amigo.findSeguidos").setParameter("usuarioSigue", idUsuario).getResultList();
		int tam=listaAmigos.size();
		List<Usuario> listaUsuarios=new ArrayList<Usuario>();
		Usuario user;
		for(int i=0;i<tam;i++)
	...
	public int getLikes(int idPublicacion){
		int tam=0;
		List<Like> listaLikes=(List<Like>)em.createNamedQuery("Like.findPublicacion").setParameter("idPublicacion", idPublicacion).getResultList();
		if(listaLikes!=null)
			tam=listaLikes.size();
		return tam;
	}
	...
	public boolean getLiked(int idPublicacion){
		boolean liked=false;
		try{
		Like like=(Like) em.createNamedQuery("Like.findUsuario").setParameter("idPublicacion", idPublicacion).setParameter("idUser", usuario.getIdUser()).getSingleResult();
		}catch(NoResultException e){
			liked=true;
		}
	...
	//COMPROBAR SI SE SIGUE AL USUARIO
		...
		public boolean comprobarSeguido(int idUser){
			...
			try{
			Amigo a=(Amigo)em.createNamedQuery("Amigo.findSeguidor").setParameter("usuarioSigue", usuario.getIdUser()).setParameter("usuarioSeguido", idUser).getSingleResult();
			}catch(NoResultException e){
				comprobar=true;	
			}
	...
	// EDITAR PERFIL
		...
		public void editarPublicacion(Publicacion publicacion){
			...
			Publicacion publicacion2=em.find(Publicacion.class, publicacion.getIdPublicacion());
		...
		public void addNotificacionLike(int idPublicacion){
			...
			notificacion.setUsuario1(em.find(Usuario.class, publicacion.getUsuario().getIdUser()));
			List<Notificacion> listaNot=em.createNamedQuery("Notificacion.findAll").getResultList();
			int tam=listaNot.size();
			if(tam>0)
				notificacion.setIdNotificacion(listaNot.get(tam-1).getIdNotificacion()+1);
			else
		...
		public void removeNotificacionLike(int idPublicacion){
			Notificacion notificacion=(Notificacion) em.createNamedQuery("Notificacion.findLike").setParameter("idPublicacion",idPublicacion).setParameter("idUser", usuario.getIdUser()).setParameter("accion", "like").getSingleResult();
		...
		// ELIMINAR NOTIFICACION SEGUIR
				...
				public void removeNotificacionSeguir(int idUser){
					Notificacion notificacion=(Notificacion) em.createNamedQuery("Notificacion.findSeguir").setParameter("idUser1",idUser).setParameter("idUser2", usuario.getIdUser()).setParameter("accion", "seguir").getSingleResult();
					em.remove(notificacion);
				}
				...
				public List<Notificacion> getListaNotificaciones(){
					return (List<Notificacion>)em.createNamedQuery("Notificacion.findUsuario").setParameter("idUser", getUsuario().getIdUser()).getResultList();
				}
				
				//BUSCAR COMENTARIOS DE UNA PUBLICACION
				@SuppressWarnings("unchecked")
				public List<Comentario> getListaComentariosPublicacion(int idPublicacion){
					return (List<Comentario>)em.createNamedQuery("Comentario.findPublicaciones").setParameter("idPublicacion", idPublicacion).getResultList();
				}
				...
				public void addNotificacionComentario(){
					...
					List<Comentario>listaCom=(List<Comentario>)em.createNamedQuery("Comentario.findAll").getResultList();
					...
					;
					int idUser=em.find(Publicacion.class, comentario.getPublicacion()).getUsuario().getIdUser();
				...
				/*public String getNombreCompleto(int idUser)
				{
					String n;
					Usuario usuario=em.find(Usuario.class, idUser);
					n=usuario.getNombreCompleto();
					return n;
				}*/
