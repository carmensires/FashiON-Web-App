...
public class ListadoBean {
	...
	public List<Publicacion> getListaPublicaciones() {
		List<Publicacion> listaPublicaciones = ejb
				.getListaPublicacionesSeguidos();
	...
	public List<Publicacion> getListaPublicacionesUsuario(Usuario user) {
		...
		List<Publicacion> listaPublicaciones = ejb.getPublicacionesUsuario(user
				.getIdUser());
		if (user.getTipoPerfil() == 1) {
			if (ejb.comprobarSeguido(user.getIdUser())
					&& user.getIdUser() != ejb.getUsuario().getIdUser()) {
	...
	public List<Comentario> getListaComentariosPublicacion(int idPublicacion) {
		List<Comentario> listaComentarios = ejb
				.getListaComentariosPublicacion(idPublicacion);
	...
	public List<Usuario> getListaUsuariosBusqueda() {
		...
		if (lista.size() != 0) {
			Collections.sort(lista, new Comparator<Usuario>() {
				@Override
				public int compare(final Usuario usuario1,
						final Usuario usuario2) {
					return usuario1.getUserName().compareTo(
		...
		if (listaNombreCompleto.size() != 0) {
			Collections.sort(listaNombreCompleto, new Comparator<Usuario>() {
				...
				public int compare(final Usuario usuario1,
						final Usuario usuario2) {
					return usuario1.getNombreCompleto().compareTo(
							usuario2.getNombreCompleto());
				}
			});
